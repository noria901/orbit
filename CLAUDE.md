...
- それでも確証が持てない場合は、推測で直すのではなく、実機のコンソールログ・スクリーンショットを要求してから直す。このプロジェクトで実際に一発診断できた不具合(ISSモデルの重心オフセット、地形トグルのIonトークン問題)は全部、実機ログを見てから直したものだった。
- **HTTPステータスやerrorEventが「成功」を示していても、それは「正しい中身が返ってきた」ことの証明にはならない。** GIBSの放射状の欠け問題では、`errorEvent`が0件(=取得は全部成功)なのに見た目は壊れたままという状況を「取得は成功してるから別の層(Cesiumの描画側)が悪い」と誤読し続け、実際には取得された画像の中身(ピクセル値)そのものが原因だった。疑わしいデータは可能なら実際に中身を覗く(ダウンロードして解析する、ブラウザで直接開いて見る等)。
...
### 3. GIBS(NASA衛星画像)のタイル配信 — 最終結論

**本当の原因は「衛星が1日でその地点を撮影できていない軌道間ギャップが、GIBS側で
不透明な黒ピクセルとして画像に焼き込まれていた」ことだった。** 放射状の暗い欠けは
タイル取得の失敗でもCesium側の描画バグでもなく、正しく取得された画像の中身が
文字通り黒かっただけ。これは実際にタイルをダウンロードしてピクセル値を解析して
初めて確定した(Playwrightでのレイヤー分離テストで、Natural Earth II/Blue Marbleは
綺麗に描画され、日次衛星画像レイヤーだけに欠けがあることも確認済み)。

**教訓: HTTPステータスやerrorEventだけを見て「取得成功=正常」と判断してはいけない。
正しく取得されたデータの中身(この場合はピクセル値)まで検証しないと、
本当の原因には辿り着けない。**

最終的な解決策:
- タイル方式をWMTS(タイル座標ベース)からWMS(BBOX指定)に変更。WMSはCesiumの
  標準GeographicTilingSchemeとそのまま噛み合うため、以下で試して全部ハマった
  「カスタムタイリングスキーム」「標準スキーム+customTagsでのURL座標変換」の
  ような複雑な仕組みが丸ごと不要になった。
- 日次衛星画像を `MODIS_Terra_CorrectedReflectance_TrueColor`(スワス幅2330km)から
  `VIIRS_SNPP_CorrectedReflectance_TrueColor`(スワス幅3000km)に変更。
  スワスが広い分、軌道間ギャップ自体が小さくなる。
- `parameters: { transparent: true, format: 'image/png' }` で透過PNGを要求し、
  `ImageryLayer.colorToAlpha = Cesium.Color.BLACK` `colorToAlphaThreshold = 0.1`
  で残ったギャップ(黒ピクセル)を透過させ、下層のBlue Marbleを見せる。
- 3層構成(最下層 Natural Earth II → 中間層 Blue Marble → 上層 VIIRS SNPP)自体は維持。

<details>
<summary>ここに至るまでの誤った仮説と回り道(記録として残す)</summary>

以下は全部、最終的には的外れだったと判明した仮説。同じ道を辿らないための記録。

- KVP方式(`?LAYER=...&TILEMATRIX=...`)はTileMatrixラベルの解釈がサーバー実装依存で
  ズレることがあり、実際にブロック状の低解像度描画になる不具合が出た
  (これ自体は事実。REST方式の方が安定という結論は変わらない)。
- `maximumLevel` は当てずっぽうで決めるとハマる。250mセットはlevel 8、500mセットは
  level 7が正解(NASA公式 `nasa-gibs/gibs-ml` の解像度対応表で確認)。これも事実として有効。
- 「GIBSの実グリッドは-180°〜396°/90°〜-198°(576°×288°)のパディング付き矩形だから、
  これをCesiumのカスタムタイリングスキームに反映すれば400エラーが直るはず」→
  これは400エラーの解消には効いたが(NASA公式GDAL_WMS設定例と数値が一致することも確認済み)、
  放射状の欠けの原因ではなかった。何度も「これが原因では」→検証→再現、を繰り返したが、
  実際にはタイル取得もCesium側の幾何計算も一切間違っていなかった。
  最終的にWMSへの切り替えでこの経路自体が丸ごと不要になった。
- Blue Marble自体の欠損、地形起伏トグルのIonトークン問題(これは実在した別の不具合)など、
  他の要因も同時に疑ったが、いずれも放射状の欠けの直接原因ではなかった。

</details>

- 単純な400/404はCORS/存在しないタイルの想定内挙動。`errorEvent` にリスナーを付けると
  Cesium既定の大量console.error出力を抑止できる。ただし**「errorEventが0件=正常」とは
  限らない**ことに注意(このケースがまさにそれだった)。
